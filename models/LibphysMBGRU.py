...
from models.LibphysGRU import LibphysGRU
from models.LibphysSGDGRU import LibphysSGDGRU
import theano
...
class LibphysMBGRU(LibphysGRU):
    ...
    def generate_online_predicted_vector(self, starting_signal, window_seen_by_GRU_size, uncertaintly=0.05):
        ...
        normalized_curve = np.zeros(self.signal_dim)
        for i in range(np.shape(means)[-1]):
            normalized_curve += norm.pdf(np.arange(self.signal_dim), means[:, i], uncertaintly) * next_sample_probs[means[:, i]]

        next_sample_probs = next_sample_probs[0] / np.sum(next_sample_probs[0], axis=0)
        print(next_sample_probs)
        probs += [next_sample_probs]
        # next_sample_probs = np.random.multinomial(1, next_sample_probs)
        next_sample = np.random.choice(self.signal_dim, p=next_sample_probs, size=(np.shape(next_sample_probs)[0]))
        ...
        # print(next_sample)
        # plt.show()
        return next_sample, next_sample_probs
